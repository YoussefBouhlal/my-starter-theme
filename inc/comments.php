...
namespace Mystartertheme\Blocks;
...
function comment_navigation( $location = '' ) {
	...
	$output .= '<h3 class="screen-reader-text">' . esc_html__( 'Comment navigation', 'mystartertheme_textdomain' ) . '</h3>';
	$output .= '<div class="nav-links">';
	$output .= '<div class="nav-previous">' . get_previous_comments_link( esc_html__( 'Older Comments', 'mystartertheme_textdomain' ) ) . '</div>';
	$output .= '<div class="nav-next">' . get_next_comments_link( esc_html__( 'Newer Comments', 'mystartertheme_textdomain' ) ) . '</div>';
	$output .= '</div>';
	$output .= '</nav>';

	// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Value is hardcoded and safe, not set via input.
	echo apply_filters( 'mystartertheme_comment_navigation', $output, $location );
}
...
function staff_comment_class( $classes, $class, $comment_id, $comment, $post_id ) {
	...
	$staff_roles = apply_filters( 'mystartertheme_staff_roles', $staff_roles );
...
function comment_form_button_class( $args ) {
	...
	$args['title_reply'] = __( 'Leave a comment', 'mystartertheme_textdomain' );
