...
namespace Mystartertheme\Block_Areas;
...
function block_areas() {
	...
	return apply_filters( 'mystartertheme_block_areas', $block_areas );
}
...
function register_cpt() {
	$labels = [
		'name'               => __( 'Block Areas', 'mystartertheme_textdomain' ),
		'singular_name'      => __( 'Block Area', 'mystartertheme_textdomain' ),
		'add_new'            => __( 'Add New', 'mystartertheme_textdomain' ),
		'add_new_item'       => __( 'Add New Block Area', 'mystartertheme_textdomain' ),
		'edit_item'          => __( 'Edit Block Area', 'mystartertheme_textdomain' ),
		'new_item'           => __( 'New Block Area', 'mystartertheme_textdomain' ),
		'view_item'          => __( 'View Block Area', 'mystartertheme_textdomain' ),
		'search_items'       => __( 'Search Block Areas', 'mystartertheme_textdomain' ),
		'not_found'          => __( 'No Block Areas found', 'mystartertheme_textdomain' ),
		'not_found_in_trash' => __( 'No Block Areas found in Trash', 'mystartertheme_textdomain' ),
		'parent_item_colon'  => __( 'Parent Block Area:', 'mystartertheme_textdomain' ),
		'menu_name'          => __( 'Block Areas', 'mystartertheme_textdomain' ),
	];
	...
	acf_add_local_field_group(array(
        ...
        'fields' => array(
            array(
                ...
                'label' => 'Block Area',
                'name' => 'mystartertheme_block_area',
                'type' => 'select',
                'instructions' => '',
                'required' => 0,
                'conditional_logic' => 0,
...
function limit_block_posts( $post_id ) {
	...
	$block_area = get_post_meta( $post_id, 'mystartertheme_block_area', true );
	if ( empty( $block_area ) ) {
		return;
	}
	...
	$others = new \WP_Query( [
		...
		'meta_key' => 'mystartertheme_block_area',
	...
	if ( ! empty( $others->posts ) ) {
		foreach( $others->posts as $other_id ) {
			delete_post_meta( $other_id, 'mystartertheme_block_area' );
		}
	}
}
...
function admin_column( $columns ) {
	...
	foreach( $columns as $key => $value ) {
		...
		if ( 'title' === $key ) {
			$new[ 'mystartertheme_block_area'] = 'Assigned to';
		}
	}
	return $new;
}
...
function admin_column_value( $column_name, $post_id ) {
	if ( 'mystartertheme_block_area' === $column_name ) {
		echo block_area_name( get_post_meta( get_the_ID(), 'mystartertheme_block_area', true ) );
	}
}
...
function show( $block_area = '', $echo = true ) {
	$output = '';
	$args   = [ 'post_type' => 'block_area', 'posts_per_page' => 1, 'post_status' => 'publish', 'meta_key' => 'mystartertheme_block_area', 'meta_value' => esc_attr( $block_area ) ];
	...
	if( $loop->have_posts() ):
		while( $loop->have_posts() ):
			...
			$output .= apply_filters( 'mystartertheme_the_content', $post->post_content );
			$output .= '</div>';
		endwhile;
	endif;
	wp_reset_postdata();
