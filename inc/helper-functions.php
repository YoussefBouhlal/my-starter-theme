...
global $wp_embed;
add_filter( 'mystartertheme_the_content', array( $wp_embed, 'run_shortcode' ), 8 );
add_filter( 'mystartertheme_the_content', array( $wp_embed, 'autoembed'     ), 8 );
add_filter( 'mystartertheme_the_content', 'do_blocks', 9 );
add_filter( 'mystartertheme_the_content', 'wptexturize' );
add_filter( 'mystartertheme_the_content', 'convert_chars' );
add_filter( 'mystartertheme_the_content', 'wpautop' );
add_filter( 'mystartertheme_the_content', 'shortcode_unautop' );
add_filter( 'mystartertheme_the_content', 'do_shortcode' );
 ...
 */
function mystartertheme_first_term( $args = [] ) {
 ...
 */
function mystartertheme_class( $base_classes, $optional_class, $conditional ) {
 ...
 */
function mystartertheme_icon( $atts = array() ) {
	...
	if ( true === $atts['force'] ) {
		...
		global $mystartertheme_icons;
		if ( empty( $mystartertheme_icons[ $atts['group'] ] ) ) {
			$mystartertheme_icons[ $atts['group'] ] = [];
		}
		if ( empty( $mystartertheme_icons[ $atts['group'] ][ $atts['icon'] ] ) ) {
			$mystartertheme_icons[ $atts['group'] ][ $atts['icon'] ] = 1;
		} else {
			$mystartertheme_icons[ $atts['group'] ][ $atts['icon'] ]++;
		}
 ...
 * Icon Definitions
 */
function mystartertheme_icon_definitions() {
	global $mystartertheme_icons;

	if ( empty( $mystartertheme_icons ) ) {
		return;
	}

	echo '<svg style="display:none;"><defs>';
	foreach ( $mystartertheme_icons as $group => $icons ) {
		foreach ( $icons as $icon => $count ) {
			echo mystartertheme_icon( [ 'icon' => $icon, 'group' => $group, 'defs' => true ] );
		}
	}
	echo '</defs></svg>';
}
add_action( 'wp_footer', 'mystartertheme_icon_definitions', 20 );
 ...
 */
function mystartertheme_has_action( $hook ) {
	ob_start();
	do_action( $hook );
	$output = ob_get_clean();
	return ! empty( $output );
 ...
 */
function mystartertheme_button( $field = [], $atts = [] ) {
 ...
 * Is Block Area
 */
function mystartertheme_is_block_area( $block_area = '', $post_id = '' ) {
	return 'block_area' === get_post_type( $post_id ) && $block_area === get_post_meta( $post_id, 'mystartertheme_block_area', true );
}

/**
 * Render Search
 */
function mystartertheme_render_search() {
