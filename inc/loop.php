 ...
 * Default Loop
 */
function mystartertheme_default_loop() {
	...
	if ( have_posts() ) :
		...
		tha_entry_before();
		$context = apply_filters( 'mystartertheme_empty_loop_partial_context', 'no-content' );
...
add_action( 'tha_content_loop', 'mystartertheme_default_loop' );


/**
 * Entry Title
 */
function mystartertheme_entry_header() {
	if ( mystartertheme_has_h1_block() ) {
		add_filter( 'render_block', 'mystartertheme_entry_header_in_content', 10, 2 );

	} else {
		do_action( 'mystartertheme_entry_title_before' );
		echo '<h1 class="entry-title">' . esc_html( get_the_title() ) . '</h1>';
		do_action( 'mystartertheme_entry_title_after' );
	}
}
add_action( 'tha_entry_top', 'mystartertheme_entry_header' );
 ...
 */
function mystartertheme_entry_header_in_content( $output, $block ) {
	if ( 'core/heading' === $block['blockName'] && ! empty( $block['attrs'] ) && ! empty( $block['attrs']['level'] ) && 1 === $block['attrs']['level'] ) {
		...
		do_action( 'mystartertheme_entry_title_after' );
		$output = ob_get_clean();
		remove_filter( 'render_block', 'mystartertheme_entry_header_in_content', 10 );
	}
 ...
 */
function mystartertheme_has_h1_block( $blocks = array() ) {
	...
	foreach ( $blocks as $block ) {
		...
		if ( 'acf/header' === $block['blockName'] ) {
			...
			$inner_h1 = mystartertheme_has_h1_block( $block['innerBlocks'] );
			if ( $inner_h1 ) {
				return true;
			}
		}
